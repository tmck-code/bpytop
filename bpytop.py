...
class CpuCollector(Collector):
    ...
    @classmethod
    def _collect(cls):
        cls.cpu_usage[0].append(ceil(mean(termux_cpu.CPUGlobalStateReader(CORES).load_percentages())))
        ...
        cpu_percent = termux_cpu.CPUGlobalStateReader(CORES).load_percentages()
        for x in ["upper", "lower"]:
            if getattr(CONFIG, "cpu_graph_" + x) == "total":
                setattr(cls, "cpu_" + x, cls.cpu_usage[0])
            else:
            ...
            if len(getattr(cls, "cpu_" + x)) > Term.width * 4:
                del getattr(cls, "cpu_" + x)[0]
        for n, thread in enumerate(termux_cpu.CPUGlobalStateReader(CORES).load_percentages(), start=1):
