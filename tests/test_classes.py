...
def test_Box_calc_sizes():
    ...
    assert (
        CpuBox.width
        == MemBox.width + ProcBox.width
        == NetBox.width + ProcBox.width
        == 80
    )
    assert (
        CpuBox.height + ProcBox.height
        == CpuBox.height + MemBox.height + NetBox.height
        == 25
    )
...
def test_MemCollector_collect():
    ...
    assert (
        isinstance(MemCollector.string["total"], str)
        and MemCollector.string["total"] != ""
    )
